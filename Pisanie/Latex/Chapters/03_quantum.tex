\newpage
\section{Kwantowe sieci neuronowe}

Obliczenia kwantowe oraz kwantowe przetwarzanie danych są relatywnie nowymi dyscyplinami, w~których zasady fizyki kwantowej są wykorzystywane do przechowywania i~przetwarzania danych \cite{Nakahara2013}.
Uczenie maszynowe przyczyniło się do rewolucyjnych zastosowań zarówno w~społeczeństwie, jak i~w świecie nauki, jednakże wraz z~ciągle słabnącym prawem Moore’a\footnote{Prawo empiryczne, wynikające z obserwacji, że ekonomicznie optymalna liczba tranzystorów w układzie scalonym zwiększa się w kolejnych latach zgodnie z trendem wykładniczym} poszukiwane są nowe ścieżki celem podtrzymania dynamizmu rozwoju technologicznego. Jedną z~tych ścieżek jest kwantowe uczenie maszynowe, które podąża za rozwojem pierwszych urządzeń kwantowych, których powstanie jest skutkiem próby ominięcia ograniczeń technologicznych i~termodynamicznych klasycznej komputacji \cite{training_deep_quantum_nn}.
...
Obliczenia kwantowe zasadniczo różnią się dwoma kluczowymi aspektami:
\begin{itemize}
    \item \textbf{Superpozycja stanu.} Układ może znajdować się w~dwóch stanach jednocześnie, dopóki nie zostanie wykonany jego pomiar.
    \item \textbf{Splątanie stanów.} Pomiar jednego układu wpływa na stan innych z~nim splątanych układów.
\end{itemize}

Wykorzystanie kwantowego podejścia do opisu danych umożliwia znaczące przyspieszenie obliczeń oraz m.in. działanie systemów kryptograficznych odpornych na niektóre typy ataków \cite{Nakahara2013}.

\subsection{Qubit jako nośnik informacji}

Qubit (ang. \textit{quantum bit}) jest podstawową jednostką informacji w~obliczeniach kwantowych, odpowiednikiem bitu w~obliczeniach klasycznych. W~przeciwieństwie do bitu, który może przyjmować jedną z~dwóch wartości (0 lub 1), qubit może znajdować się w~stanie superpozycji obu tych wartości jednocześnie. Stany qubitu można zwizualizować na sferze Blocha (o promieniu równym 1) w~układzie kartezjańskim. Zgodnie z~konwencją: oś z~jest skierowana w~górę, a~oś y~w~bok \cite{quantum_intro}.
Szczególne przypadki stanów qubitu opisano są w tabeli \ref{tab:qubit_bloch}.

\begin{table}[H]
\caption{\textbf{Reprezentacja podstawowych stanów qubitów na sferze Blocha.}}
...
\begin{figure}[H]
    ...
    \begin{minipage}[b]{0.48\textwidth}
        \centering
        \caption{\textbf{Sfera Blocha z~zaznaczonymi podstawowymi stanami qubitów.}}
        \includegraphics[width=\linewidth,height=6cm,keepaspectratio]{Figures/bloch_from_introduction_pg_76.png}
        \label{fig:bloch_sphere}
        \caption*{Źródło: \cite{quantum_intro}}
    \end{minipage}
    \hfill
    \begin{minipage}[b]{0.48\textwidth}
        \centering
        \caption{\textbf{Sfera Blocha z~kątami $\theta$ oraz $\phi$ \cite{quantum_intro}}.}
...
Interpretowalnym wynikiem pomiaru qubitu jest zawsze jedna z~dwóch wartości: 0 lub 1. Prawdopodobieństwo uzyskania każdej z~tych wartości zależy od stanu, w~jakim znajduje się qubit przed pomiarem. Na przykład, jeśli qubit jest w~stanie $\ket{+}$, to istnieje 50\% prawdopodobieństwa uzyskania wyniku 0 i~50\% prawdopodobieństwa uzyskania wyniku 1 podczas pomiaru.

Dla stanu kwantowego $\ket{\psi}$:
\begin{equation}
...
\begin{equation}
\label{eq:quantum_prob}
P(0) = |\braket{0|\psi}|^2 = |\alpha|^2, \quad
P(1) = |\braket{1|\psi}|^2 = |\beta|^2,
\end{equation}

gdzie $P(0)$ to prawdopodobieństwo uzyskania wyniku 0, a~$P(1)$ to prawdopodobieństwo uzyskania wyniku 1. Wartości $\alpha$ i~$\beta$ są amplitudami prawdopodobieństwa dla stanów $\ket{0}$ i~$\ket{1}$ odpowiednio, przy czym muszą spełniać warunek normalizacji: $|\alpha|^2 + |\beta|^2 = 1$.

Jednocześnie równanie \ref{eq:quantum_state} można wyrazić:
\begin{equation}
    \ket{\psi} = \cos{\frac{\theta}{2}} \ket{0} + e^{i\phi}\sin{\frac{\theta}{2}} \ket{1}, \quad
\end{equation}

gdzie $\theta$ to odległość zenitalna, czyli miara kąta między wektorem $\vec{OM}$ ($O$ -- początek układu współrzędnych sfery Blocha, $M$ punkt na sferze Blocha) a~dodatnią półosią $OZ$, natomiast $\phi$ to długość azymutalna, czyli miara kąta między rzutem prostokątnym wektora $\vec{OM}$ na płaszczyznę OXY a~dodatnią półosią OX. (vide rysunek \ref{fig:bloch_angles}). Właściwości tych miar kątowych uzasadniają skalowanie zbioru wykorzystanych w~niniejszej pracy danych do zakresu $[-1,1]$, które umożliwia wprowadzenie danych diagnostycznych do stanu kwantowego jako kąty wyrażone w~radianach.
...
\subsection{Bramki kwantowe jako operacje na obwodach kwantowych}

Bramki kwantowe stanowią odpowiednik klasycznych bramek logicznych w~obliczeniach kwantowych. Są transformacją, liniowym mapowaniem stanu qubitu w~inne stany. Po przekształceniu całkowite prawdopodobieństwo stanu wciąż wynosi 1. W~ujęciu algebry liniowej bramki kwantowe są macierzami. Tabela \ref{tab:bramki_kwantowe} przedstawia wybrane bramki kwantowe wraz z~ich działaniem na bazie, reprezentacją macierzową oraz interpretacją funkcjonalną \cite{quantum_intro}.
Każda bramka kwantowa jest macierzą unitarną, co oznacza:
\[
U^{\dagger} U~= U~U^{\dagger} = I,
\]
gdzie $U$ jest macierzą bramki kwantowej, $U^{\dagger}$ jest jej sprzężeniem hermitowskim (transpozycja sprzężenie zespolone), a~$I$ jest macierzą jednostkową.


\begin{table}[H]
\caption{\textbf{Przykładowe bramki kwantowe działające na pojedynczym qubicie.}}
...
\resizebox{\textwidth}{!}
{
    ...
    \begin{tabular}{l l l l}
        ...
        \hline
        Hadamard $H$ & $\begin{aligned} H\ket{0} &= \frac{\ket{0} + \ket{1}}{\sqrt{2}} \\ H\ket{1} &= \frac{\ket{0} - \ket{1}}{\sqrt{2}} \end{aligned}$ & $H = \frac{1}{\sqrt{2}} \begin{pmatrix} 1 & 1 \\ 1 & -1 \end{pmatrix}$ & Obrót o $\pi$ jednocześnie wokół osi $x$ i~$z$ \\
        \hline
    \end{tabular}
}
...
\subsection{Hybrydowe kwantowe uczenie maszynowe}
...
Równoważnie można zapisać równanie (\ref{eq:stan_wyjscie_1}) jako:
...
gdzie:
\begin{itemize}
    \item $K$ -- macierz parametrów operatora unitarnego. Dobrane tak, aby funkcja ryzyka $C$ miała początkowo jak największy dodatni gradient,
...
\begin{itemize}
    ...
    \item $\mathcal{F}(X) = \sum_{\alpha}A_{\alpha}^{\dagger}XA_{\alpha}$ -- sprzężny kanał kwantowy do $\varepsilon(X)=\sum_{\alpha}A_{\alpha}XA_{\alpha}^{\dagger}$, czyli kanał przejścia z~$l+1$ do $l$
\end{itemize}

Własnością tej struktury sieci jest fakt, że dla wyznaczenia $K_j^l$ dla warstwy $l-1$ oraz $l$, potrzebne są jedynie informacje z~tych dwóch warstw. Oznacza to, że aktualizacja lokalnych parametrów może być wykonywana lokalnie dla każdej warstwy, bez konieczności wyznaczania każdego z~parametrów sieci, jak ma to miejsce w~klasycznych sieciach neuronowych. Koszt obliczeniowy zatem skaluje się z~szerokością sieci, a~nie z~jej głębokością, co jest korzystne z~punktu widzenia efektywności obliczeniowej.

Ograniczeniem jest jednak liczba qubitów ze względu na eksponencjalnie rosnącą przestrzeń Hilberta. Autorzy w~tym podejściu dostrzegają kluczową zaletę: ograniczenie występowania tzw. zanikania gradientu (ang. \textit{barren plateau}) podczas trenowania sieci kwantowych, co jest powszechnym problemem w~klasycznych sieciach neuronowych oraz innych architekturach QNN.
...
\begin{tcolorbox}[title=Algorytm trenowania głębokiej sieci kwantowej,
    ...
    \begin{enumerate}
        ...
        Krok $s=0$. Losowe inicjalizowanie parametrów bramek kwantowych w~sieci.
        \item \textbf{Feedforward} 
        Wykonanie działań dla każdej pary stanów treningowych $\ket{\phi_x^{in}},\ket{\phi_x^{out}}$:
        \begin{enumerate}
            \item Wyznaczenie stanu: 
            \[\mathcal{T} = \rho_x^{l-1}(s)\otimes \ket{0...0}_l\bra{0...0} \]
            \item Zastosowanie operatorów 
            \[\mathcal{O} = \prod_{j=m_l}^{1}U_j^l(s)\mathcal{T}\prod_{j=1}^{m_l}{U_j^l(s)}^{\dagger} \]
            \item Wyznaczenie śladu częściowego \[\rho_x^l(s) = \operatorname{tr}_{l-1}(\mathcal{O}) \]
            \item Przechowanie stanu $\rho_x^l(s)$ 
        \end{enumerate}
        ...
        \begin{enumerate}
            \item Obliczenie funkcji kosztu $C(s)$ zgodnie z~równaniem \ref{eq:f_kosztu}.
            \item Wyznaczenie każdej macierzy $K_j^l(s)$ dla każdej warstwy $l$ i~każdego perceptronu $j$ w~tej warstwie:
            \begin{equation}
            ...
            \end{itemize}
            \item \textbf{Aktualizacja operatorów unitarnych} 
            Wykonanie operacji unitarnych dla każdej warstwy $l$ i~każdego perceptronu $j$:
        \[U_j^l(s+1) = e^{i\epsilon K_j^l(s)}U_j^l(s)\]
            \item Zwiększenie kroku $s = s~+ \epsilon$ 
        \end{enumerate}
        \item \textbf{Iteracja} Powtórzenie kroków 2 i~3 aż do osiągnięcia maksymalnej wartości funkcji kosztu.
