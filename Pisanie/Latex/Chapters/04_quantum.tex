...
\subsection{Czym jest kubit?}\cite{quantum_intro}
...
\begin{figure}[H]
    ...
    \caption{Sfera Blocha z zaznaczonymi podstawowymi stanami qubitów.}
...
Własnością tej struktury sieci jest fakt, że dla wyznaczenia $K_j^l$ dla warstwy $l-1$ oraz $l$, potrzebne są jedynie informacje z tych dwóch warstw. Oznacza to, że aktualizacja parametrów może być wykonywana lokalnie dla każdej warstwy, bez konieczności propagacji wstecznej przez całą sieć, jak ma to miejsce w klasycznych sieciach neuronowych. $K$ zatem skaluje się z szerokością sieci a nie z jej głębokością, co jest korzystne z punktu widzenia efektywności obliczeniowej.

Ograniczeniem jest jednak ilość qubitów ze względu na ekspotencjalnie rosnącą przestrzeń Hilberta. Autorzy w tym podejściu dostrzegają kluczową zaletę jaką jest brak występowania tzw. zanikania gradientu (ang. \textit{barren plateau}) podczas trenowania sieci kwantowych, co jest powszechnym problemem w klasycznych sieciach neuronowych oraz innych architekturach QNN.
...
\begin{tcolorbox}[title=Algorytm trenowania głębokiej sieci kwantowej,
    ...
    \begin{enumerate}
        ...
        \item \textbf{Feedforward} Dla każdej pary stanów treningowych $\ket{\phi_x^{in}},\ket{\phi_x^{out}}$ wykonaj:
        \begin{enumerate}
            \item Wyznacz \[\mathcal{T} =\rho_x^{l-1}(s)\otimes \ket{0...0}_l\bra{0...0} \]
            \item Zastosuj operatory \[\mathcal{O} = \prod_{j=m_l}^{1}U_j^l(s)\mathcal{T}\prod_{j=1}^{l}{U_j^l(s)}^{\dagger} \]
            \item Wyznacz ślad częściowy \[\rho_x^l(s) = \operatorname{tr}_{l-1}(\mathcal{O}) \]
            \item Przechowaj $\rho_x^l(s)$ 
        \end{enumerate}
        ...
        \begin{enumerate}
            \item Oblicz funkcję kosztu $C(s)$ zgodnie z równaniem \ref{eq:f_kosztu}.
            \item Wyznacz każdą macierz $K_j^l(s)$ 
            \begin{equation}
            ...
            \end{itemize}
            \item \textbf{Aktualizacja operatorów unitarnych} Dla każdej warstwy l i każdego perceptronu j wykonaj:
        \[U_j^l(s+1) = e^{i\epsilon K_j^l(s)}U_j^l(s)\]
            \item Zwiększ krok $s = s + \epsilon$ 
        \end{enumerate}
        \item \textbf{Iteracja} Powtarzaj kroki 2 i 3 aż do osiągnięcia maksymalnej wartości funkcji kosztu.
